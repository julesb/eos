local deathray = pd.Class:new():register("deathray")

local eos = require("eos")
local v2 = require("vec2")

function deathray:initialize(sel, atoms)
  self.inlets = 2
  self.outlets = 2
  self.npoints = 100
...
function deathray:in_1_bang()
  ...
  local head = {
    x = arscalex * self.x1amp
      * simplex.noise2d(t*self.x1freq*arscaley + self.x1phase, 2311.323),
    y = arscaley * self.y1amp
      * simplex.noise2d(t*self.y1freq*arscalex + self.y1phase, 1234.567)
  }
  ...
  eos.addpoint(out, self.headprev.x, self.headprev.y, 0,0,0, 2)
  ...
  if self.drawtrail then
    ...
    for i=1,self.npoints do
      ...
      t = t - self.trailstep
      trailx = arscalex * self.x1amp
             * simplex.noise2d(t*self.x1freq*arscaley + self.x1phase, 2311.323)
      traily = arscaley * self.y1amp
             * simplex.noise2d(t*self.y1freq*arscalex + self.y1phase, 1234.567)
    ...
    eos.addpoint(out, trailx, traily, 0, 0, 0, 4)
  ...
  self.psys.update(1.0/60.0)
  deathray:to_xyrgb(out, self.psys.particles)
...
function deathray:in_2_npoints(x)
  if x[1] >= 10 then
    self.npoints = x[1]
...
function deathray:in_2(sel, atoms)
...
function deathray:to_xyrgb(out, particles)
