...
function IldaFrame:parse(frameOffset, bytes)
  ...
  while (i < dataStartIdx + datalen) do
    ...
    if formatCode == IldaHeader.ILDA_3D_INDEXED
    or formatCode == IldaHeader.ILDA_2D_INDEXED then
      ...
      y = y / 32767.0
      z = z / 32767.0
      status = string.byte(recBytes, formatCode == IldaHeader.ILDA_3D_INDEXED
                           and 7 or 5)
      colIdx = 1 + string.byte(recBytes, formatCode == IldaHeader.ILDA_3D_INDEXED
                           and 8 or 6)
    ...
    elseif formatCode == IldaHeader.ILDA_3D_RGB or formatCode == IldaHeader.ILDA_2D_RGB then
      ...
      st_last = (status & (1 << 7)) >> 7
      st_blank = (status & (1 << 6)) >> 6
      local b = (string.byte(recBytes, formatCode == IldaHeader.ILDA_3D_RGB
                             and 8 or 6) & 0xff) / 255.0
      local g = (string.byte(recBytes, formatCode == IldaHeader.ILDA_3D_RGB
                             and 9 or 7) & 0xff) / 255.0
      local r = (string.byte(recBytes, formatCode == IldaHeader.ILDA_3D_RGB
                             and 10 or 8) & 0xff) / 255.0
      rgb = {r, g, b}
      colIdx = -1
      p = IldaPoint:new(x, y, z, colIdx, rgb, st_blank == 1, st_last == 1)
      table.insert(self.points, p)
...
function IldaFrame:toString()
  return "frame " .. (self.header.frameNumber + 1) .. "/"
                  .. self.header.totalFrames .. ": "
                  .. self.header:getFormatString() .. " "
                  .. self.header.numRecords
                  .. " points "
                  .. "[" .. self.header.name .. "|"
                  .. self.header.companyName .. "]"
end
